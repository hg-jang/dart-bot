...
module.exports = {
  /**
   * data의 csv 파일 읽어서 문자열로 변환
   * @param {string} fileName 
   * @returns {string}
   */
  csvFileToString: function(fileName) {
    try {
  ...
  /**
   * json을 csv 문자열로 변환
   * https://curryyou.tistory.com/257
  ...
  jsonToCsv: function (json_data) {
    ...
    titles.forEach((title, index) => {
      csv_string += (index !== titles.length - 1 ? `${title},` : `${title}${lineBreak}`);
    });
    ...
    json_array.forEach((content, index) => {
      ...
      csv_string += (index !== json_array.length - 1 ? `${row}${lineBreak}` : `${row}`);
    })
   ...
   * @returns {object} json
   */
  csvToJson: function (csv_string) {
    // 1. 문자열을 줄바꿈으로 구분 => 배열에 저장
    // const rows = csv_string.split("\r\n");

    // 줄바꿈을 \n으로만 구분해야하는 경우, 아래 코드 사용
    const rows = csv_string.split(lineBreak);

    // 2. 빈 배열 생성: CSV의 각 행을 담을 JSON 객체임
    const jsonArray = [];

    // 3. 제목 행 추출 후, 콤마로 구분 => 배열에 저장
    let header = rows[0].split(",").map((col) => col.trim());
